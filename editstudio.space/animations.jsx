...
// ────────────────────────────────────────────────────────────────
// 5) LASHES — cream lash-fan eye on the page's dark ground; ambient
//    flutter, periodic blink, drifting fallen lashes ("make a wish")
//    and sparkles. The eye closes on horizontal transition.
// ────────────────────────────────────────────────────────────────
function LashAnim({ progress = 0, speed = 1 }) {
  ...
  // Fallen lashes drifting down + sparkles, stable seeds.
  const drifters = useMemo(() => Array.from({ length: 12 }).map((_, i) => ({
    x: (i * 269) % 1000, delay: (i * 431) % 7000,
    dur: 6000 + (i * 157) % 3500, curl: 18 + (i * 13) % 22
  })), []);
      ...
      <svg viewBox="0 0 1000 1400" preserveAspectRatio="xMidYMid slice"
        ...
        <defs>
          <radialGradient id="lashGlow" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#8a6cc0" stopOpacity="0.5" />
            <stop offset="55%" stopColor="#67509a" stopOpacity="0.22" />
            <stop offset="100%" stopColor="#67509a" stopOpacity="0" />
          </radialGradient>
        </defs>
        ...
        <g transform={`rotate(${sway} 500 600)`}>
          ...
          <g transform={`translate(500 604) scale(1 ${open}) translate(-500 -604)`}>
            ...
            <path d="M140 600 Q500 380 860 600" fill="none"
                  stroke="#f5f0e8" strokeWidth="5" strokeLinecap="round" opacity="0.85" />
            {lashes.map((l, i) => {
              ...
              return (
                <g key={i} transform={`rotate(${flut} ${l.px} ${l.py})`}>
                  <path d={`M${l.px} ${l.py} Q ${l.px + l.nx * l.len * 0.55} ${l.py + l.ny * l.len * 0.62} ${tipX + l.curl} ${tipY}`}
                        fill="none" stroke="#f5f0e8" strokeWidth="2.6"
          ...
          <path d="M140 600 Q500 700 860 600" fill="none"
                stroke="#f5f0e8" strokeWidth="4" strokeLinecap="round"
                opacity={closed * 0.9} />

          {/* lower-lid hint */}
          <path d="M240 640 Q500 730 760 640" fill="none"
                stroke="#f5f0e8" strokeWidth="1.6" opacity={0.22 * (1 - closed)} />
        </g>

        {/* fallen lashes drifting down */}
        {drifters.map((d, i) => {
          const phase = ((t + d.delay) % d.dur) / d.dur;
          const y = -40 + phase * 1500;
          const x = d.x + Math.sin(t / 1000 + i * 1.3) * 34;
          ...
          return (
            <path key={i}
                  ...
                  fill="none" stroke="#f5f0e8" strokeWidth="1.8"
        ...
        {sparks.map((s, i) => {
          ...
          return (
            <g key={i} transform={`translate(${s.x} ${s.y})`} opacity={tw * 0.55}>
              <path d={`M0 ${-r} L${r * 0.32} 0 L0 ${r} L${-r * 0.32} 0 Z`} fill="#e8ddf5" />
              <path d={`M${-r} 0 L0 ${r * 0.32} L${r} 0 L0 ${-r * 0.32} Z`} fill="#e8ddf5" />
            </g>
          );
        })}
        ...
        <text x="500" y="1060" textAnchor="middle"
              fontFamily="Fraunces, serif" fontStyle="italic" fontWeight="300"
              fontSize="170" fill="#f5f0e8" opacity="0.07" letterSpacing="-3">
          flutter
        </text>
      </svg>
    </div>
