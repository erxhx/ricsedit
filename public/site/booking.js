      ...
      function renderField(f) {
        ...
        if (f.type === "checkbox_group") {
          return React.createElement(
            ...
            React.createElement(
              "div",
              { style: { display: "flex", flexWrap: "wrap", gap: "6px 16px", marginTop: 8 } },
              (f.options || []).map(function(opt) {
                var sel = (responses[f.id] || []).indexOf(opt) >= 0;
                return React.createElement(
                  "label",
                  { key: opt, style: { display: "flex", alignItems: "center", gap: 7, cursor: "pointer", fontFamily: "var(--body)", fontSize: 13, color: "var(--ink-soft)", userSelect: "none", minWidth: "45%" } },
                  React.createElement(
                    "span",
                    { onClick: function() {
                      toggleCheck(f.id, opt);
                    }, style: { flex: "0 0 auto", width: 18, height: 18, borderRadius: 3, border: "1px solid " + (sel ? "var(--ink)" : "var(--rule)"), background: sel ? "var(--ink)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.12s" } },
